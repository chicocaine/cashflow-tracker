...
CREATE TABLE operation (
    ...
    description     TEXT             CHECK (description IS NULL OR length(description) <= 128),
    operation_type  INTEGER NOT NULL CHECK (operation_type IN (0, 1, 2, 3, 4)), -- OperationType: 0=CREATE,1=UPDATE,2=DELETE,3=SUSPEND,4=ARCHIVE
...
-- ----------------------------------------------------------------------------
-- device_metadata
-- Simple key/value store, e.g. for per-device sync state (last synced
-- counter, device name, etc). WITHOUT ROWID since key is already a unique,
-- non-null TEXT primary key and rows are small — avoids the extra rowid
-- index/indirection.
-- ----------------------------------------------------------------------------
...
-- Note: This SQL source file is only a form of documentation and 
-- a source of truth for the application. It is not utilized in, or does
